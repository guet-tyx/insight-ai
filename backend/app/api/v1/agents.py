...
class ReviewRequest(BaseModel):
    action: str = Field(
        description="approve / reject / revise", pattern="^(approve|reject|revise)$"
    )
...
class _RunStore:
    ...
    def mark_running(self, run_id: str) -> None:
        with self._lock:
            run = self._runs[run_id]
            self._runs[run_id] = run.model_copy(update={"status": "running", "draft_report": None})

    def finish(
        self, run_id: str, status: str, report: str | None = None, error: str | None = None
    ) -> None:
        with self._lock:
            run = self._runs[run_id]
            self._runs[run_id] = run.model_copy(
...
async def _execute_run(run_id: str, instruction: str) -> None:
    ...
    try:
        async for _mode, data in g.astream(
            _run_inputs(instruction),
        ...
        logger.exception("agents run %s 失败", run_id)
...
async def _continue_run(run_id: str, feedback: dict[str, Any]) -> None:
    ...
    try:
        ...
        if "429" in str(exc) or "RateLimit" in str(exc):
            ...
            return
        run_store.finish(run_id, "failed", error=f"{type(exc).__name__}: {exc}"[:500])
        logger.exception("agents run %s 恢复执行失败", run_id)
...
async def create_run(payload: RunRequest, _: User = Depends(get_current_user)) -> RunStartResponse:
    ...
    _bg_tasks.add(asyncio.create_task(_execute_run(run_id, payload.instruction)))
...
async def stream_run(run_id: str, _: User = Depends(get_current_user)) -> StreamingResponse:
    ...
    async def event_stream():
        ...
        while True:
            ...
            if (datetime.now(UTC) - run.created_at).total_seconds() > 900:
